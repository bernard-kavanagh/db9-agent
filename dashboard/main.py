...
async def api_leads(
    ...
    try:
        ...
        leads = get_leads(conn, geo=geo, country=country, sub_region=sub_region, global_region=global_region,
...
async def api_export(
    ...
    for lead in leads:
        ...
        writer.writerow([
            ...
            lead.get("country", ""), lead.get("global_region", ""),
            lead.get("sub_region", ""), lead.get("geo", ""),
            lead.get("industry", ""), lead.get("company_size", ""),
            lead.get("description", ""), lead.get("db9_pain", ""),
            lead.get("db9_use_case", ""), lead.get("fit_score", ""),
            lead.get("status", ""), contacts,
        ])
    output.seek(0)
    filename = "db9-leads-score" + str(min_score) + "+"
    if country:
        filename += "-" + country.lower().replace(" ", "-")
    elif sub_region:
        filename += "-" + sub_region.lower().replace(" ", "-")
    elif geo:
        filename += "-" + geo.lower()
    filename += ".csv"
...
async def api_search(
    ...
    try:
        ...
        if region:  kw_filters.append("sub_region = %s");  kw_params.append(region)
        with conn.cursor() as cur:
            cur.execute(
    ...
    try:
        ...
        try:
            lead_id = _upsert_lead(
                ...
                global_region="Manual", sub_region="Manual", geo=geo,
