...
def lead_text(lead: dict) -> str:
    parts = [
        lead.get("db9_pain") or "",
        lead.get("db9_use_case") or "",
...
def backfill_embeddings(conn) -> tuple[int, int]:
    with conn.cursor() as cur:
        cur.execute("""
            SELECT id, db9_pain, db9_use_case, description, industry
            FROM leads
...
def hybrid_search(
    ...
    if region:
        conditions.append("l.sub_region = %s")
    ...
    sql = f"""
        SELECT
            l.id, l.company_name, l.website, l.country, l.sub_region, l.global_region, l.geo,
            l.industry, l.company_size, l.description,
            l.db9_pain, l.db9_use_case, l.fit_score, l.status, l.created_at,
            ...
            , 1) AS similarity_pct,
            CASE
                WHEN l.db9_pain     LIKE %s THEN 1
                WHEN l.db9_use_case LIKE %s THEN 1
        ...
        ORDER BY
            ...
            + (CASE WHEN l.db9_pain LIKE %s OR l.db9_use_case LIKE %s THEN 0.3 ELSE 0 END)
            DESC
        LIMIT %s
    """.format(where=where)
